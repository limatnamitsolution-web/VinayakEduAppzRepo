...
{
    public class DapperMasterConfigRepository : IDapperMasterConfigRepository
    {
        private readonly DapperDbContext _db;
        private readonly IDapperUnitOfWork _uow;

        public DapperMasterConfigRepository(DapperDbContext db, IDapperUnitOfWork uow)
        {
        ...
        public async Task<MasterConfig?> GetByConfigurationAsync(int academicId, string configuration, CancellationToken cancellationToken = default)
        {
            ...
            {
                var result = await _uow.Connection.QueryFirstOrDefaultAsync<MasterConfig>(new CommandDefinition(sql, new { AcademicId = academicId, Configuration = configuration }, _uow.Transaction, cancellationToken: cancellationToken));
                return result;
            }

            using var conn = await _db.CreateOpenConnectionAsync(cancellationToken).ConfigureAwait(false);
            return await conn.QueryFirstOrDefaultAsync<MasterConfig>(new CommandDefinition(sql, new { AcademicId = academicId, Configuration = configuration }, cancellationToken: cancellationToken));
        }
